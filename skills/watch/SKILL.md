...
homepage: https://github.com/sager-coder/claude-video
repository: https://github.com/sager-coder/claude-video
...
On macOS with Homebrew, it auto-installs `ffmpeg` and `yt-dlp`. On Linux (including Cowork sandboxes), it auto-installs via apt-get/dnf when running as root or with passwordless sudo, and via pipx/pip --user for yt-dlp; otherwise it prints the exact install commands. On Windows, it prints the install commands. It scaffolds `~/.config/watch/.env` with commented placeholders at `0600` perms, and writes `SETUP_COMPLETE=true` once deps + a key are in place so the next session knows this user has already been through the wizard.
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/watch.py" "<source>"
```
...
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/watch.py" video.mp4 --start 50 --end 60

# Zoom into 2:15 → 2:45 at 3 fps (90 frames)
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/watch.py" "$URL" --start 2:15 --end 2:45 --fps 3

# From 1h12m to the end of the video
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/watch.py" "$URL" --start 1:12:00
```
...
- **Setup preflight failed** → run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/setup.py"` (auto-installs ffmpeg/yt-dlp via brew on macOS or apt-get/dnf+pipx on Linux when permissions allow, scaffolds the `.env`). For API key, ask the user via `AskUserQuestion` and write it to `~/.config/watch/.env`.
